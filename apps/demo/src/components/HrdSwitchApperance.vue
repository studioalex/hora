...
<template>
 <div class="HrdNavBarAppearance">
    <button class="HrdSwitch HrdSwitchAppearance" type="button" role="switch" aria-label="toggle dark mode" @click="switchMode">
      <span class="check">
        <span class="icon">
...
.HrdNavBarAppearance {
  ...
  &:before {
    ...
    width: 1px;
    height: 24px;
    background-color: var(--hr-color-divider-light);
    content: "";
  }
  ...
  .HrdSwitch {
    ...
    height: 22px;
    flex-shrink: 0;
    border: 1px solid var(--hr-color-divider);
    background-color: var(--hr-color-bg-mute);
...
.dark .HrdNavBarAppearance {
  .HrdSwitch {
    .check {
      transform: translateX(18px);
      background-color: var(--hr-color-black);

      .icon {
        svg {
          fill: var(--hr-color-text-1);
